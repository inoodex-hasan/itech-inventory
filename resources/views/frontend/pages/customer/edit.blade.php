            ...
            <form method="POST" action="{{ route('customers.update', $customer->id) }}">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $customer->name) }}" placeholder="Enter full name" required>
                        @error('name')
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="tel" class="form-control @error('phone') is-invalid @enderror" name="phone" value="{{ old('phone', $customer->phone) }}" pattern="[0-9]{11}" maxlength="11" placeholder="Enter 11-digit phone number" required>
                        @error('phone')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email', $customer->email) }}" placeholder="Enter email address">
                        @error('email')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('address') is-invalid @enderror" name="address" rows="3" placeholder="Enter complete address..." required>{{ old('address', $customer->address) }}</textarea>
                        @error('address')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
